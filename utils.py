...
# Using azureopenai for vector embeddings
...
# Grab table and column names and datatypes from oracle db
def fetch_schema():
    ...
    try:
        ...
        # Only using V_ARC tables unless updated
...
# Extracting possible table and column names from user prompt
# Can optimize this (doesn't need amt_tables or amt_columns)
...
# From possible table names, match to actual table names in trackwise db
...
# From possible table names, match to actual table names from each extracted table in trackwise db
...
# Generated query execution on trackwise db
...
# Reading in jsonl file for verified queries
# Change this to use established db system
def read_verify_df():
    verify_df_path = 'data/query_verification.jsonl'
    df_list = []
    with open(verify_df_path, 'r') as f:
        for line in f:
