...
function nlp2sql(userInput) {
    ...
    .then(data => {
        ...
        .then(() => {
            ...
            setTimeout(addConfirmationMessage, 750);
        });
...
function addConfirmationMessage() {
    ...
    var aiParagraph = document.createElement('p');
    aiParagraph.textContent = 'Is this query correct?';
    aiParagraph.classList.add('confirmation-text');

    var checkmarkButton = document.createElement('button');
    checkmarkButton.textContent = '✔️';
    checkmarkButton.classList.add('checkmark-button');
    checkmarkButton.addEventListener('click', () => handleConfirmation(aiMessage, true));

    var crossButton = document.createElement('button');
    crossButton.textContent = '❌';
    crossButton.classList.add('cross-button');
    crossButton.addEventListener('click', () => handleConfirmation(aiMessage, false));

    aiMessage.appendChild(aiParagraph);
    aiMessage.appendChild(checkmarkButton);
    aiMessage.appendChild(crossButton);
    document.getElementById('chat-box').appendChild(aiMessage);

    scrollToBottom();
}

function handleConfirmation(messageElement, isConfirmed) {
    const textElement = messageElement.querySelector('.confirmation-text');
    const buttons = messageElement.querySelectorAll('button');
    
    if (textElement) textElement.classList.add('fade-out'); 
    ...
    setTimeout(() => {
        ...
        newMessage.classList.add('fade-in-message');
        newMessage.textContent = isConfirmed
            ? 'Your response has been added to TRACKWISE search\'s knowledge!'
            : 'Your response has been discarded.';
...
function verifyQuery() {
    const userQuery = lastUserInput.trim(); 
    const sqlQueryElement = document.querySelector('.message.ai-message p');
    const sqlQuery = sqlQueryElement ? sqlQueryElement.textContent.trim() : '';

    console.log('User Query:', userQuery);
    console.log('SQL Query:', sqlQuery);

    if (userQuery === '' || sqlQuery === '') {
        alert('Cannot verify. Please ensure the queries are not empty.');
        return;
    }
    ...
    .then(data => {
        if (data.error) {
            alert('Error verifying query: ' + data.error);
        } else {
            alert(data.message);
        }
    })
    .catch(error => {
        ...
        alert('Error verifying query. Please try again later.');
    });
}
